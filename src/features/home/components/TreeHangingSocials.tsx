...
interface HangingSocialItem {
  ...
  stringHeight: number; // string length in pixels
...
const HANGING_SOCIALS: HangingSocialItem[] = [
  {
    ...
    // Placed directly on the left bough of the tree
    branchLeft: "26%",
    branchTop: "27%",
    stringHeight: 90,
    swingDuration: 4.8,
  ...
  },
  {
    ...
    // Placed directly on the inner-left upper bough of the tree
    branchLeft: "38%",
    branchTop: "20%",
    stringHeight: 110,
    swingDuration: 5.6,
  ...
  },
  {
    ...
    // Placed directly on the inner-right upper bough of the tree
    branchLeft: "62%",
    branchTop: "21%",
    stringHeight: 115,
    swingDuration: 4.4,
  ...
  },
  {
    ...
    // Placed directly on the right bough of the tree
    branchLeft: "74%",
    branchTop: "28%",
    stringHeight: 85,
    swingDuration: 5.0,
    ...
    borderColor: "group-hover:border-brand-violet/70",
    badgeBg: "group-hover:bg-brand-violet/10",
    icon: (
      <svg className="w-3.5 h-3.5 fill-current text-brand-ink" viewBox="0 0 24 24">
      ...
      {HANGING_SOCIALS.map((item) => (
        ...
        >
          ...
          >
            ...
            {/* Suspended thread / string ("suta") */}
            <div
              className="w-[1.5px] bg-gradient-to-b from-emerald-400 via-emerald-300/60 to-zinc-400 dark:to-zinc-300 relative"
              style={{ height: `${item.stringHeight}px` }}
            >
            ...
            <div className="relative flex items-center justify-center -mb-1 z-10">
              <div className="w-2.5 h-2.5 rounded-full border-[1.5px] border-emerald-400/90 bg-white/40 dark:bg-zinc-900/80 backdrop-blur-xs shadow-xs" />
            </div>
            ...
            <motion.a
              ...
              whileTap={{ scale: 0.95 }}
              className={`pointer-events-auto cursor-pointer group relative inline-flex items-center gap-1.5 sm:gap-2 px-3 sm:px-3.5 py-1.5 rounded-2xl bg-white/90 dark:bg-zinc-900/95 backdrop-blur-xl border border-brand-ink/15 dark:border-white/20 shadow-[0_4px_16px_rgba(0,0,0,0.12),inset_0_1px_1px_rgba(255,255,255,0.4)] transition-all duration-300 ${item.borderColor} ${item.badgeBg}`}
            >
              {/* Hole punch graphic at top center of card */}
              <span className="absolute -top-1 left-1/2 -translate-x-1/2 w-1.5 h-1.5 rounded-full bg-brand-surface dark:bg-zinc-800 border border-brand-ink/20" />
              ...
              {/* Platform Name Label */}
              <span className="font-heading text-xs sm:text-[13px] font-semibold text-brand-ink tracking-wide group-hover:text-brand-violet transition-colors select-none">
                {item.name}
              </span>

              {/* Little interactive indicator leaf dot */}
              <span className="w-1.5 h-1.5 rounded-full bg-emerald-400/80 group-hover:bg-emerald-400 group-hover:scale-125 transition-all shadow-[0_0_6px_rgba(52,211,153,0.8)]" />
            </motion.a>
          </motion.div>
        </div>
      ))}
