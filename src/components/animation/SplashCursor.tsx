...
export function SplashCursor({
  ...
  useEffect(() => {
    ...
    if (!contextData || !contextData.gl || !contextData.ext) {
      return;
    }
    ...
    function getWebGLContext(targetCanvas: HTMLCanvasElement) {
      ...
      let gl = targetCanvas.getContext("webgl2", params) as any;
      const isWebGL2 = !!gl;
      if (!isWebGL2) {
        gl =
    ...
    function compileShader(
      ...
      const sourceWithKeywords = addKeywords(source, keywords);
      const shader = gl.createShader(type);
      if (!shader) return null;
      gl.shaderSource(shader, sourceWithKeywords);
    ...
    function clickSplat(pointer: Pointer) {
      ...
      color.g *= 8.0;
      color.b *= 8.0;
      const dx = 12 * (Math.random() - 0.5);
      const dy = 28 * (Math.random() - 0.5);
    ...
    function hexToRGB(hex: string) {
      ...
      let parsed = val;
      if (val.length === 3)
        parsed = val[0] + val[0] + val[1] + val[1] + val[2] + val[2];
  ...
  return (
    <div
      className={`fixed inset-0 pointer-events-none z-[9990] overflow-hidden ${className}`}
      aria-hidden="true"
    >
      <canvas
        ref={canvasRef}
